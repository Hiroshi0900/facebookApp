<template>
    <div class='bg-white h12 px-4 flex items-center border-b border-gray-400 shadow-sm'>
		<!-- 検索バー -->
		<div class='w-1/3'>
		    <div class='flex'>
				<router-link to='/'>
				...
				<div class='ml-2 relative'>
					...
					class='rounded-full pl-8 w-56 bg-gray-200 h-8 
		...
		<div class='w-1/3 flex justify-center items-center h-full'>
		    <router-link to='/' class='px-6 border-b-2 border-white h-full flex items-center' v-bind:class='{currentView:homeView}'>
			...
			<router-link :to="'/users/'+ authUser.data.user_id" class='px-6 border-b-2 border-white h-full flex items-center' v-bind:class='{currentView:userView}'>
			    <img :src='authUser.data.attributes.profile_image.data.attributes.path' alt=''
		...
		<div class='w-1/3 flex justify-end'>
			<img src='../../../public/images/logout.svg' v-on:click='logoutHandler'
			class="w-8 h-8 object-cover cursor-pointer" viewBox="0 0 24 24">
...
<script>
    ...
    export default {
		...
		mounted(){
			...
			// console.log(this.userView,this.homeView);
